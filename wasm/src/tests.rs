...
//
// crypto_vectors.json is generated by the Go SDK and consumed here, so any
// drift between the two protocol implementations fails one of the two suites.
// Regenerate with: go test ./pkg/client -run TestCryptoVectors -update

const VECTORS_JSON: &str = include_str!("../../crypto_vectors.json");

fn vectors() -> serde_json::Value {
    serde_json::from_str(VECTORS_JSON).expect("crypto_vectors.json is not valid JSON")
}

fn field<'a>(vector: &'a serde_json::Value, name: &str) -> &'a str {
    vector[name]
        .as_str()
        .unwrap_or_else(|| panic!("vector field {name} is missing"))
}

fn field_b64(vector: &serde_json::Value, name: &str) -> Vec<u8> {
    b64d(field(vector, name)).unwrap_or_else(|e| panic!("vector field {name}: {e}"))
}

#[test]
fn test_vector_argon2_params_match() {
    let params = &vectors()["argon2"];
    assert_eq!(params["memoryKiB"].as_u64().unwrap(), ARGON2_M_COST as u64);
    assert_eq!(params["timeCost"].as_u64().unwrap(), ARGON2_T_COST as u64);
    assert_eq!(
        params["parallelism"].as_u64().unwrap(),
        ARGON2_P_COST as u64
    );
    assert_eq!(params["keyLen"].as_u64().unwrap(), KEY_SIZE as u64);
}

#[test]
fn test_vectors_text() {
    let vectors = vectors();
    let cases = vectors["text"].as_array().expect("text vectors");
    assert!(!cases.is_empty(), "no text vectors");

    for case in cases {
        let name = field(case, "name");
        let passphrase = field(case, "passphrase");
        let salt = field_b64(case, "salt");
        let nonce = field_b64(case, "nonce");
        let header = field_b64(case, "header");
        let plaintext = field(case, "plaintext");

        assert_eq!(
            hash_password_internal(passphrase, &salt),
            field(case, "passwordHash"),
            "{name}: passwordHash mismatch"
        );

        let ct = xchacha_encrypt(passphrase, &nonce, &salt, &header, plaintext.as_bytes()).unwrap();
        assert_eq!(
            b64e(&ct),
            field(case, "ciphertext"),
            "{name}: ciphertext mismatch"
        );

        let expected_ct = field_b64(case, "ciphertext");
        let pt = xchacha_decrypt(passphrase, &nonce, &salt, &header, &expected_ct).unwrap();
        assert_eq!(
...
fn test_vectors_file() {
    let vectors = vectors();
    let cases = vectors["file"].as_array().expect("file vectors");
    assert!(!cases.is_empty(), "no file vectors");
    ...
    for case in cases {
        ...
        let nonce = field_b64(case, "nonce");
        let header = field_b64(case, "header");
        let meta_nonce = field_b64(case, "metaNonce");
        let file_data = field_b64(case, "fileData");
        ...
        let enc_file = xchacha_encrypt(passphrase, &nonce, &salt, &header, &file_data).unwrap();
        assert_eq!(
            b64e(&enc_file),
            field(case, "encryptedFile"),
            "{name}: encryptedFile mismatch"
        );

        // The metadata plaintext must serialize identically in both languages.
        let metadata_json = serde_json::json!({
            "file_name": field(case, "fileName"),
            "file_type": field(case, "fileType"),
        })
        .to_string();
        assert_eq!(
            metadata_json,
            field(case, "metadataJson"),
            "{name}: metadata JSON mismatch"
        );

        let enc_meta = xchacha_encrypt(
            passphrase,
            &meta_nonce,
            &salt,
            &header,
        ...
        .unwrap();
        let mut blob = meta_nonce.clone();
        blob.extend_from_slice(&enc_meta);
        assert_eq!(
            b64e(&blob),
            field(case, "encryptedMetadata"),
            "{name}: encryptedMetadata blob mismatch"
        );

        let expected_blob = field_b64(case, "encryptedMetadata");
        let (blob_nonce, blob_ct) = expected_blob.split_at(NONCE_SIZE);
        let meta_bytes = xchacha_decrypt(passphrase, blob_nonce, &salt, &header, blob_ct).unwrap();
        assert_eq!(String::from_utf8(meta_bytes).unwrap(), metadata_json);

        let expected_file = field_b64(case, "encryptedFile");
        let decrypted =
            xchacha_decrypt(passphrase, &nonce, &salt, &header, &expected_file).unwrap();
        assert_eq!(decrypted, file_data, "{name}: file plaintext mismatch");
    }
}

#[test]
fn test_vectors_display_passphrase_split() {
    let vectors = vectors();
    let cases = vectors["displayPassphrase"]
        .as_array()
        .expect("displayPassphrase vectors");
    assert!(!cases.is_empty(), "no display passphrase vectors");

    for case in cases {
        let display = field(case, "display");
        let (salt_b64, passphrase) = display.split_at(SALT_B64_LEN);
        assert_eq!(salt_b64, field(case, "salt"));
        assert_eq!(passphrase, field(case, "passphrase"));
        assert_eq!(b64d(salt_b64).unwrap().len(), SALT_SIZE);
    }
}
